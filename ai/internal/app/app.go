...
import (
	...
	"github.com/kothagpt/kotha/ai/internal/tui/theme"
)
...
func New(ctx context.Context, conn *sql.DB, provider config.ConfigProvider) (*App, error) {
	q := db.New(conn)
	...
	app.initTheme(provider)

	// Initialize LSP clients in the background
	go app.initLSPClients(ctx, provider)
...
func (app *App) initTheme(provider config.ConfigProvider) {
	cfg := provider.Get()
