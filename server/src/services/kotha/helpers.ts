...
import { KothaContext } from './types.js'
import { KOTHA_MODE_PROMPT } from './constants.js'
...
export function getKothaMode(input: unknown): KothaMode | undefined {
  try {
    ...
    return inputNumber as KothaMode
  } catch (error) {
    console.error('Error parsing Kotha mode:', error)
    return undefined
  }
}

export function detectKothaMode(transcript: string): KothaMode {
  const words = transcript.trim().split(/\s+/)
  const firstFiveWords = words.slice(0, 5).join(' ').toLowerCase()

  return firstFiveWords.includes('hey kotha')
    ? KothaMode.EDIT
  ...
  switch (mode) {
    case KothaMode.EDIT:
      return (
        ...
        // advancedSettingsHeaders.editingPrompt || KOTHA_MODE_PROMPT[KothaMode.EDIT]
        KOTHA_MODE_PROMPT[KothaMode.EDIT]
      )
    case KothaMode.TRANSCRIBE:
      return (
        advancedSettingsHeaders.transcriptionPrompt ||
        KOTHA_MODE_PROMPT[KothaMode.TRANSCRIBE]
      )
    default:
      return KOTHA_MODE_PROMPT[KothaMode.TRANSCRIBE]
  }
}
