...
import (
	...
	"github.com/kothagpt/kotha/ai/internal/lsp/watcher"
)

func (app *App) initLSPClients(ctx context.Context, provider config.ConfigProvider) {
	cfg := provider.Get()

	// Initialize LSP clients
	for name, clientConfig := range cfg.LSP {
		// Start each client initialization in its own goroutine
		go app.createAndStartLSPClient(ctx, provider, name, clientConfig.Command, clientConfig.Args...)
	}
	logging.Info("LSP clients initialization started in background")
}

// createAndStartLSPClient creates a new LSP client, initializes it, and starts its workspace watcher
func (app *App) createAndStartLSPClient(ctx context.Context, provider config.ConfigProvider, name string, command string, args ...string) {
	...
	_, err = lspClient.InitializeLSPClient(initCtx, provider.WorkingDirectory())
	if err != nil {
	...
	go app.runWorkspaceWatcher(watchCtx, name, workspaceWatcher, provider)
}

// runWorkspaceWatcher executes the workspace watcher for an LSP client
func (app *App) runWorkspaceWatcher(ctx context.Context, name string, workspaceWatcher *watcher.WorkspaceWatcher, provider config.ConfigProvider) {
	defer app.watcherWG.Done()
	defer logging.RecoverPanic("LSP-"+name, func() {
		// Try to restart the client
		app.restartLSPClient(ctx, name, provider)
	})

	workspaceWatcher.WatchWorkspace(ctx, provider.WorkingDirectory())
	logging.Info("Workspace watcher stopped", "client", name)
}

// restartLSPClient attempts to restart a crashed or failed LSP client
func (app *App) restartLSPClient(ctx context.Context, name string, provider config.ConfigProvider) {
	...
	app.createAndStartLSPClient(ctx, provider, name, clientConfig.Command, clientConfig.Args...)
