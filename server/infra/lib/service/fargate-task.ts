...
export function createFargateTask(
  ...
  const fargateTaskRole = new IamRole(scope, 'KothaFargateTaskRole', {
  ...
  const taskExecutionRole = new IamRole(scope, 'KothaTaskExecRole', {
    assumedBy: new ServicePrincipal('ecs-tasks.amazonaws.com'),
    managedPolicies: [
  ...
  taskDefinition.addContainer(containerName, {
    ...
    logging: new AwsLogDriver({
      streamPrefix: 'kotha-server',
      logGroup: config.serverLogGroup,
    }),
  })

  const taskLogsPolicy = new Policy(scope, 'KothaTaskLogsPolicy', {
    statements: [
      new PolicyStatement({
        actions: [
          'logs:CreateLogGroup',
        ...
        resources: [
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/kotha/${config.stageName}/client`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/kotha/${config.stageName}/client:log-stream:*`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/kotha/${config.stageName}/server`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/kotha/${config.stageName}/server:log-stream:*`,
        ],
      }),
    ],
  })
