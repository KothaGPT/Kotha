...
export const handler = async (_event: Event) => {
  // Templates
  await request('/_index_template/kotha-client-logs', 'PUT', clientTemplate)
  await request('/_index_template/kotha-server-logs', 'PUT', serverTemplate)

  // Apply ISM policy for both patterns
  try {
    await request(
      '/_plugins/_ism/policies/kotha-retain-forever',
      'PUT',
      ismPolicy,
    )
  } catch (e: any) {
    ...
    template: {
      ...
      settings: {
        ...
        'index.plugins.index_state_management.policy_id':
          'kotha-retain-forever',
      },
    },
  })
  await request(
    '/_index_template/kotha-client-logs',
    'PUT',
    addPolicy(clientTemplate),
  )
  await request(
    '/_index_template/kotha-server-logs',
    'PUT',
