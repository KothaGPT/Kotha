...
create_windows_installer() {
    ...
    docker run --rm --platform linux/amd64 \
      ...
      --env VITE_KOTHA_VERSION="${VITE_KOTHA_VERSION}" \
      ...
      bash -c "
        ...
        if ls dist/Kotha-*.exe 1> /dev/null 2>&1; then
          echo 'Copying versioned installer to static Kotha-Installer.exe for CDN'
          cp dist/Kotha-*.exe dist/Kotha-Installer.exe
        fi
      "
...
main() {
    ...
    print_status "Starting Kotha $PLATFORM build process..."
    echo
