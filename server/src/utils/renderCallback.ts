...
<html lang="en">
  <head>
    ...
    <title>Authentication Successful - Kotha</title>
    <style>
      * {
        margin: 0;
        padding: 0;
  ...
  <body>
    <div>
      ...
      <button class="button" onclick="openApp()">Open Kotha</button>
    </div>
    ...
    <script>
      function openApp() {
        ...
        if (authCode && state) {
          window.location.href = \`kotha://auth/callback?code=\${authCode}&state=\${state}\`
        } else {
