...
if ! command -v mc &> /dev/null; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        echo "  docker run --rm -it --entrypoint=/bin/sh minio/mc -c \"mc alias set local http://host.docker.internal:9000 minioadmin minioadmin && mc mb local/kotha-audio-storage\""
        exit 1
    fi
fi
...
BUCKET_NAME=${BLOB_STORAGE_BUCKET:-kotha-blob-storage}
